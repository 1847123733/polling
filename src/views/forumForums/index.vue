...
<!--              inactive-text="禁止"-->
        ...
        <el-table-column prop="updatetime" label="更改时间" align="center" show-overflow-tooltip></el-table-column>
        <el-table-column prop="memo" label="备注" align="center" show-overflow-tooltip></el-table-column>
        <el-table-column label="操作" align="center" width="210">
          <template slot-scope="scope">
            <el-button size="mini" icon="el-icon-edit" type="amend" @click="beginUpdate(scope.row)" :disabled="userrole == 2">修改</el-button>
            <el-button size="mini" icon="el-icon-delete" type="danger" @click="beginRemove(scope.row)" :disabled="userrole == 2">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
            ...
            <div style="float: left;width: 50%;">
              <el-form-item label="所属单位：" prop="unitname" style="margin-bottom: 30px">
                <el-select
                  ...
                  <el-option v-for="item in util.slice(0, rangeNumber)" :key="item.index" :label="item.unitname" :value="item.unitid"></el-option>
                </el-select>
  ...
  export default {
    ...
    watch: {
      ...
      search(newVal, oldVal) {
        if (newVal) {
          this.util = this.stashList
          this.util = this.util.filter(item => {
            if (item.unitname.includes(newVal)) {
              return item
            }
          })
        } else {
